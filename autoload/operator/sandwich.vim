...
function! s:act_initialize(cursor, modmark) dict abort  "{{{
...
"}}}
function! s:act_add_pair(buns, stuff, undojoin) dict abort "{{{
  ...
  if s:is_valid_4pos(target) && s:is_equal_or_ahead(target.head2, target.head1)
    ...
    try
      ...
      let [is_linewise[0], indent[0], head] = s:add_former(a:buns, pos, opt, a:undojoin)
...
"}}}
function! s:act_delete_pair(stuff, modified) dict abort  "{{{
  let target  = a:stuff.target
  let edges   = a:stuff.edges
  let modmark = self.modmark
  let opt     = self.opt
...
"}}}
function! s:act_replace_pair(buns, stuff, undojoin, modified) dict abort "{{{
  let target   = a:stuff.target
  let edges    = a:stuff.edges
  let modmark  = self.modmark
  let opt      = self.opt
  ...
  if s:is_valid_4pos(target) && s:is_ahead(target.head2, target.tail1)
    ...
    try
      ...
      let [deletion[0], is_linewise[0], indent[0], head] = s:replace_former(a:buns[0], former_head, former_tail, within_a_line, opt, a:undojoin)
...
function! s:stuff_initialize(cursor, modmark, count) dict abort  "{{{
  let self.active = 1
  let self.acts = map(range(a:count), 'deepcopy(s:act)')
...
"}}}
function! s:stuff_match(recipes, opt) dict abort "{{{
  if !self.active
    return 0
  endif
...
"}}}
function! s:stuff__match_recipes(recipes, opt, ...) dict abort "{{{
...
"}}}
function! s:stuff__match_edges(recipes, opt, edge_chars) dict abort "{{{
...
"}}}
function! s:stuff_skip_space() dict abort  "{{{
  if self.active
...
let s:stuff = {
      ...
      \   '_match_recipes': function('s:stuff__match_recipes'),
      \   '_match_edges'  : function('s:stuff__match_edges'),
...
function! s:operator_initialize(kind, motionwise) dict abort "{{{
  ...
  if self.state
    let self.basket = map(range(self.n), 'deepcopy(s:stuff)')
  else
...
"}}}
function! s:operator_fill() dict abort  "{{{
  let lack = self.n - len(self.basket)
  if lack > 0
    let fillings = map(range(lack), 'deepcopy(s:stuff)')
...
"}}}
function! s:operator_add_once(i, recipe) dict abort  "{{{
...
"}}}
function! s:operator_delete_once(i) dict abort "{{{
  let modified = 0
  for j in range(self.n)
    let stuff = self.basket[j]
    if stuff.active
...
"}}}
function! s:operator_replace_once(i, recipe) dict abort  "{{{
  let buns = s:get_buns(a:recipe, self.opt.of('expr'))
  let undojoin = a:i == 0 || self.state == 0 ? 0 : 1
  let modified = 0
  for j in range(self.n)
...
"}}}
function! s:operator_set_target() dict abort  "{{{
  for i in range(self.n)
    let stuff = self.basket[i]
...
"}}}
function! s:operator_skip_space(i) dict abort "{{{
  let opt = self.opt
...
"}}}
function! s:operator_query() dict abort  "{{{
...
"}}}
function! s:operator_match(i) dict abort  "{{{
...
"}}}
function! s:operator_show(hi_group) dict abort "{{{
...
"}}}
function! s:operator_quench() dict abort "{{{
  if self.opt.of('highlight')
    for i in range(self.n)
      let stuff = self.basket[i]
      call stuff.quench()
...
"}}}
function! s:operator_blink(hi_group, duration, ...) dict abort "{{{
  if self.opt.of('highlight')
    let clock = deepcopy(s:clock)
    let hi_exited = 0
...
"}}}
function! s:operator_recipe_fill(recipe, count) dict abort  "{{{
...
let s:operator = {
      ...
      \     'fill'      : function('s:operator_recipe_fill'),
      \   },
      \   'cursor': {
      \     'head'      : copy(s:null_pos),
      ...
      \   'fill'        : function('s:operator_fill'),
      \   'set_target'  : function('s:operator_set_target'),
      \   'skip_space'  : function('s:operator_skip_space'),
      \   'query'       : function('s:operator_query'),
      \   'match'       : function('s:operator_match'),
      \   'show'        : function('s:operator_show'),
      \   'quench'      : function('s:operator_quench'),
      \   'blink'       : function('s:operator_blink'),
      \   'add'         : function('s:operator_add'),
      \   'add_once'    : function('s:operator_add_once'),
      \   'delete'      : function('s:operator_delete'),
      \   'delete_once' : function('s:operator_delete_once'),
      \   'replace'     : function('s:operator_replace'),
      \   'replace_once': function('s:operator_replace_once'),
